...
func TestStory022_DisplayDueDateInOverview(t *testing.T) {
	...
	taskWithDueDate := todo.NewFull("Submit report", todo.PriorityA, false, nil, nil, &dueDate, nil, nil, nil, nil)
...
func TestStory022_DisplayOverdueInOverview(t *testing.T) {
	...
	overdueTask := todo.NewFull("Overdue task", todo.PriorityA, false, nil, nil, &overdueDate, nil, nil, nil, nil)
...
func TestStory022_DueDateExactlyTodayNotOverdue(t *testing.T) {
	...
	taskDueToday := todo.NewFull("Task due today", todo.PriorityA, false, nil, nil, &todayDate, nil, nil, nil, nil)
...
func TestStory022_DueDatesInformationalOnly(t *testing.T) {
	...
	overdueDate := time.Date(2026, 1, 15, 0, 0, 0, 0, time.UTC)
	lowPriorityOverdue := todo.NewFull("Low priority task", todo.PriorityD, false, nil, nil, &overdueDate, nil, nil, nil, nil)
...
func TestStory022_DifferentYearFormatting(t *testing.T) {
	...
	taskWithDifferentYearDueDate := todo.NewFull("Task from past year", todo.PriorityA, false, nil, nil, &dueDate, nil, nil, nil, nil)
...
func TestStory022_DueDateInFocusedInputMode(t *testing.T) {
	...
	repository := memory.NewRepository()
	dueDate := time.Date(2026, 1, 25, 0, 0, 0, 0, time.UTC)
	taskWithDueDate := todo.NewFull("Task with due date", todo.PriorityA, false, nil, nil, &dueDate, nil, nil, nil, nil)
